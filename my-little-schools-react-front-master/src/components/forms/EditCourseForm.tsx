...
export function EditCourseForm({ data }: CourseFormProps) {
  ...
  const handleSubmit = async (event: { preventDefault(): void; }) => {
    ...
    const response = await api.put(`course/${data?.id}`, formData);
    response.status === 200 ? alert("Curso atualizado com sucesso") : alert("Erro ao atualizar curso");
          ...
          <div className="sm:col-span-3">
            ...
            <div className="mt-2">
              <select
                ...
                className="block w-full rounded-md px-2 border-0 py-2 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:max-w-xs sm:text-sm sm:leading-6"
